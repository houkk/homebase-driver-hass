...
var homebase_util = require('homebase-util');
...
module.exports = function (env) {
  // homebase init
  homebase_util.utilInit(100, env && env.logger);
  var logger = homebase_util.logger;
  var errorHandler = homebase_util.errorHandler;
    ...
    command: function(command, params) {
      if (command === 'login') {
        ...
        return hass.states.list()
          ...
          .catch(err => {
            logger.error("Get hass entities states error: ", err)
            return Promise.reject(errorHandler({
              errorName: err.errorName || '',
              debugInfo: err.debugInfo || ''
            }, err));
        ...
        logger.error('Command %s not support.', command)
        return Promise.reject(errorHandler({
          errorName: 'E_NOT_SUPPORTED_DIRECTIVE'
        }))
      }
